...
def instrument_service(path: str = "instrument_service"):
    ...
    get_instrument.wire_impl_out_stub(
        ...
        map_(
            lambda x: x,
            __keys__=get_instrument.wire_impl_inputs_stub(path).instrument,
            x=instrument_by_symbol,
        ),
    )
...
def price_service(path: str = "price_service"):
    ...
    prices = map_(
        lambda i, p: make_tsd(i, p),
        price_submissions.instrument,
    ...
    get_price.wire_impl_out_stub(
        ...
        map_(
            lambda x: x,
            __keys__=get_price.wire_impl_inputs_stub(path).instrument,
            x=prices,
        ),
    )
...
def convert_price_to_currency_units(price: TSB[Price], currency_unit: TS[Unit]) -> TSB[Price]:
    ...
    with get_price(fx_rate_symbol(price.currency_unit, currency_unit)):
        return TSB[Price[float]].from_ts(
            qty=convert_units(price.qty, price.currency_unit, currency_unit),
...
def calculate_notional(positions: Position[float], currency: TS[Unit]) -> TSB[Quantity[float]]:
    return calculate_notional_tsb(
        TSB[Position[float]].from_ts(
            qty=positions.qty,
    ...
    price_in_currency = switch_(
        {
            ...
            (True, False): lambda p, c: TSB[Price[float]].from_ts(
...
def calculate_notional_tsd(positions: TSD[str, TSB[Quantity[float]]], currency: TS[Unit]) -> TSB[Quantity[float]]:
    ...
    return map_(
        lambda key, qty, c: calculate_notional(
            TSB[Position].from_ts(qty=qty.qty, unit=qty.unit, instrument=get_instrument(key)),
            c,
        ),
        positions,
        currency,
    ...
    def g(prices: TSD[str, TSB[Price[float]]]) -> TS[Quantity[float]]:
        ...
        corn_future_months = FutureContractSeries(
            spec=FutureContractSpec(
                ...
                contract_size=Quantity(5000.0, U.bushel),
        ...
        zcm5_position = Position[float](qty=100.0, unit=U.lot, instrument=zck5)
        ...
        eval_node(
            ...
            prices=[
                None,
                {
                    ...
                    "ZCK5": Price[float](qty=500.0, currency_unit=U.USX, unit=U.bushel),
                },
            ],
        )[-1]
        == (500.0 / 1.25 * 5000.0) * U.GBP
    )  # 500 USX per bushel, 5000 bushels, 1.25 USD per GBP
