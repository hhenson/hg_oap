...
def test_unit_conversion_1():
    with UnitSystem() as U:
        ...
        assert U.meter.convert(100.0, to=U.meter) == 100.0

        U.cm = DerivedUnit(primary_unit=U.meter, ratio=0.01)

        assert U.cm.convert(100.0, to=U.meter) == 1.0
        assert U.cm.convert(100.0, to=U.meter) == 1.0

        assert U.meter.convert(100.0, to=U.cm) == 10000.0

        U.meter_sq = U.meter**2
        U.cm_sq = U.cm**2

        assert U.meter_sq.convert(1.0, to=U.cm_sq) == 10000.0
        assert U.cm_sq.convert(1.0, to=U.meter_sq) == 0.0001
        ...
        assert (U.meter / U.second).convert(1.0, to=U.cm / U.minute) == 6000.0
        assert (U.cm / U.second).convert(1.0, to=U.meter / U.minute) == 60.0 / 100.0

        with pytest.raises(ValueError):
            (U.meter / U.second).convert(1.0, to=U.meter_sq)
...
def test_conversion_2():
    with UnitSystem(__prefixes__={"m": 0.001}) as U:
        ...
        assert (U.mpint**-1).convert(1.0, to=U.bushel**-1) == 1 / 0.000568 * 35.2391
...
def test_offset_units():
    with UnitSystem() as U:
        ...
        assert U.celsius.convert(0.0, to=U.celsius) == 0.0
        assert U.kelvin.convert(273.15, to=U.kelvin) == 273.15

        assert U.celsius.convert(0.0, to=U.kelvin) == 273.15
        assert U.kelvin.convert(273.15, to=U.celsius) == 0.0

        assert U.celsius.convert(100.0, to=U.kelvin) == 373.15

        U.fahrenheit = OffsetDerivedUnit(primary_unit=U.kelvin, ratio=5.0 / 9.0, offset=459.67)

        assert U.fahrenheit.convert(32.0, to=U.fahrenheit) == 32.0
        assert round(U.fahrenheit.convert(32.0, to=U.celsius), 2) == 0.0
        assert round(U.fahrenheit.convert(32.0, to=U.kelvin), 2) == 273.15
        assert round(U.celsius.convert(0.0, to=U.fahrenheit), 2) == 32.0
        assert round(U.kelvin.convert(273.15, to=U.fahrenheit), 2) == 32.0
        ...
        assert U.thermal_conductivity.convert(1.0, to=U.joule / (U.meter * U.second * U.fahrenheit.diff)) == 5.0 / 9.0

        with pytest.raises(AssertionError):  # not allowed to use an offset unit
            U.thermal_conductivity.convert(1.0, to=U.joule / (U.meter * U.second * U.fahrenheit))
...
def test_qualified_units():
    with UnitSystem() as U:
        ...
        assert U.USX.convert(100.0, to=U.USD) == 1.0
        assert U.EUX.convert(100.0, to=U.EUR) == 1.0
        assert (U.EUR / U.USD).name == "EUR/USD"  # EUR/USD

        with UnitConversionContext((1.15 * (U.USD / U.EUR),)):
            assert U.EUR.convert(1.0, to=U.USD) == 1.15
...
def test_contexts_and_conversion_factors():
    with UnitSystem() as U:
        ...
        U.second = PrimaryUnit(dimension=U.timespan)
        U.minute = DerivedUnit(primary_unit=U.second, ratio=60.0)
        U.hour = DerivedUnit(primary_unit=U.minute, ratio=60.0)

        U.velocity = U.length / U.timespan
        U.meter_per_second = U.meter / U.second

        my_speed = 2.0 * U.meter_per_second

        with UnitConversionContext((my_speed,)):
            assert U.hour.convert(1.0, to=U.meter) == 7200.0
...
def test_contexts_and_conversion_factors_2():
    with UnitSystem() as U:
        ...
        with instrument:
            assert U.lot.convert(1.0, to=U.bushel) == 10000.0
            assert U.lot.convert(1.0, to=U.cubic_meter) == 352.391
            assert U.lot.convert(1.0, to=U.mt) == 264.29325

            assert round(U.mt.convert(1.0, to=U.lot), 5) == 0.00378
            assert round(U.pound.convert(1000.0, to=U.lot), 5) == 0.00172
