...
class DGen(Item):
    ...
    def __invoke__log__(
        ...
        **kwargs,
        ...
        for d in self.__invoke__(start, end, after, before, calendar, **kwargs):
    ...
    def __invoke__(
        ...
        **kwargs,
        ...
        if is_dgen(self.date):
            after = next(
                self.date.__invoke__log__(start, end, after, before, calendar, **kwargs)
            )
        else:
            after = self.date
...
class JoinDGen(DGen):
    ...
    def __invoke__(
        ...
        **kwargs,
        ...
        g1 = self.gen1.__invoke__log__(start, end, after, before, calendar, **kwargs)
        g2 = self.gen2.__invoke__log__(start, end, after, before, calendar, **kwargs)
...
class SubSequenceDGen(DGen):
    def __init__(self, main_sequence, sub_sequence, slice=None):
        if main_sequence.cadence() in (None, Tenor("1d")):
            raise ValueError(
                f"cannot generate sub sequences from main sequence with cadence of {main_sequence.cadence()}"
            )
    ...
    def __invoke__(
        ...
        **kwargs,
        ...
        for begin in self.main_sequence.__invoke__log__(
...
class DaysOfMonthDGen(DGen):
    ...
    def __invoke__(
        ...
        **kwargs,
        ...
        for month in self.months.__invoke__log__(
            start, end, after, before, calendar, **kwargs
        ):
...
class SliceDGen(DGen):
    ...
    def __invoke__(
        ...
        **kwargs,
        ...
        yield from islice(
            self.gen.__invoke__log__(start, end, after, before, calendar, **kwargs),
            self.slice.start,
            self.slice.stop,
            self.slice.step,
        )
...
class WithCalendarDGen(DGen):
    ...
    def __invoke__(
        ...
        **kwargs,
        ...
        yield from (
            d for d in self.gen.__invoke__log__(start, end, after, before, self.calendar)
        )
...
class MonthEndDgen(DGen):
    ...
    def __invoke__(
        ...
        **kwargs,
        ...
        for d in self.gen.__invoke__log__(start, end, after, before, calendar, **kwargs):
            yield date(d.year, d.month, monthrange(d.year, d.month)[1])

    def __repr__(self):
        return f"month_end({self.gen})"


def month_end(x):
    if is_op(x):
        return lazy(month_end)(x)
    else:
        return MonthEndDgen(x)


def month_last_bday(x, calendar=None):
    if is_op(x) or is_op(calendar):
        return lazy(month_last_bday)(x, calendar)
    else:
        return roll_bwd(month_end(x), calendar)


class MonthStartDgen(MonthsDGen):
    def __init__(self, gen):
        self.gen = gen
    ...
    def __invoke__(
        ...
        **kwargs,
        ...
        for d in self.gen.__invoke__log__(start, end, after, before, calendar, **kwargs):
            yield date(d.year, d.month, 1)

    def __repr__(self):
        return f"month_start({self.gen})"


def month_start(x):
    if is_op(x):
        return lazy(month_start)(x)
    else:
        return MonthStartDgen(x)


def month_first_bday(x, calendar=None):
    if is_op(x) or is_op(calendar):
        return lazy(month_first_bday)(x, calendar)
    else:
        return roll_fwd(month_start(x), calendar)
