...
from hgraph import compute_node, add_, TS, Frame, mul_, NUMBER, div_, zero, pow_, graph, convert, \
    const, TSD, sub_, AUTO_RESOLVE, TSB


def _as_polars(value) -> pl.DataFrame:
    return value if isinstance(value, pl.DataFrame) else pl.from_arrow(value)


@graph(overloads=zero)
def zero_timed_value(
    tp: Type[TS[TIMED_VALUE]], op: object, tp_: type[TIMED_VALUE] = AUTO_RESOLVE
) -> TS[TIMED_VALUE]:
...
@graph(overloads=zero)
def zero_timed_value_frame(tp: Type[TS[Frame[TimedValue]]], op: object) -> TS[Frame[TimedValue]]:
...
def add_timed_value_frames(lhs: TS[Frame[TIMED_VALUE]], rhs: TS[Frame[TIMED_VALUE]]) -> TS[Frame[TIMED_VALUE]]:
    lhs_value = _as_polars(lhs.value)
    rhs_value = _as_polars(rhs.value)
